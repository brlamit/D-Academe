...
$courseId = $name = $description  = $imagePath = '';
$message = '';
...
if (isset($_GET['course_id'])) {
    ...
    $sql = "SELECT * FROM free_courses WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($courseId) || empty($name) || empty($description)) {
    ...
    $sql = "UPDATE free_courses SET name = ?, image = ?, description = ? WHERE id = ?";

    // Prepare the statement
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param('sssi', $name,  $imagePath, $description,  $courseId); // 'sssi' = string, string, string, integer
