...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM user_login WHERE reset_token = ? AND expiry > NOW()"); 
    ...
    if ($result->num_rows > 0) {
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if ($new_password === $confirm_password) {
                ...
                $stmt = $conn->prepare("UPDATE user_login SET password = ?, reset_token = NULL, expiry = NULL WHERE reset_token = ?");
...
<body class="min-h-screen flex items-center justify-center">
    ...
    <div id="successModal" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md text-center">
            ...
            <a href="user_login.html" class="bg-teal-600 text-white px-6 py-2 rounded-lg hover:bg-teal-700 transition-all">
                Go to Login
            </a>
        </div>
    </div>
